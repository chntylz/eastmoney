...
def check_table():
    table_exist = hdata_balance.table_is_exist() 
    my_dbg('table_exist=%d' % table_exist)
    if table_exist:
        hdata_balance.db_hdata_eastmoney_create()
...
if __name__ == '__main__':
    ...
    if int(para1):
        my_dbg('get history balance data')
        get_all = 1
    else:
        get_all = 0
    ...
    while (1):
        try:
            f_df, api_param = get_fina_data3('balance',i)
            if len(f_df):
    ...
    df.to_csv('./csv/' + nowdate.strftime("%Y-%m-%d")+ '_balance_all_df_' + str(i) + '.csv', encoding='gbk')
    try :
    ...
    if len(df) > 0:
        ...
        my_dbg('balance dataframe is null')
...
'''
df = pd.read_csv('./csv/balance_all_df_301.csv',encoding='gbk', index_col=[0], dtype={'security_code':str})
df = df.drop_duplicates(subset=['security_code', 'reportdate'], keep='first')
