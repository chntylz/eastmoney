...
class HData_eastmoney_zlpm(object):
    ...
    def table_is_exist(self):
        self.db_connect()
        self.cur.execute("select count(*) from pg_class where relname = 'eastmoney_zlpm_table' ;")
    ...
    def db_hdata_eastmoney_create(self):
        ...
        # 创建stocks表
        self.cur.execute('''
            drop table if exists eastmoney_zlpm_table;
            create table eastmoney_zlpm_table(
                stock_code varchar, 
                record_date date,
                stock_name varchar, 
                close float,
            ...
            alter table eastmoney_zlpm_table add primary key(stock_code, record_date);
            ''')
        self.conn.commit()
        self.db_disconnect()

        my_dbg("db_eastmoney_zlpm_table_create finish")
        pass
    ...
    def copy_from_stringio(self, df):
        ...
        self.db_connect()
        try:
            self.cur.copy_from(buffer, table='eastmoney_zlpm_table', sep=",")
            self.conn.commit()
    ...
    def db_get_maxdate_of_stock(self,stock_code):#获取某支股票的最晚日期
        ...
        self.db_connect()
        self.cur.execute("select max(record_date) from eastmoney_zlpm_table \
            ...
            for i in range(0,length):
                ...
                if i % each_num == 0 and i != 0:
                    if debug:
                        my_dbg(sql_cmd)
                    if(sql_cmd != ""):
                        final_cmd = "insert into eastmoney_zlpm_table ("\
                                + eastmoney_cols + \
                                " ) values "+sql_cmd+";"
                        if debug:
                            my_dbg(final_cmd)
        ...
        if data is None:
            ...
            for i in range(length):
                ...
                if i % each_num == 0 and i != 0:
                    ...
                    if len(sql_cmd):
                        final_sql = [] 
                        final_sql.append("insert into eastmoney_zlpm_table (")
    ...
    def insert_all_stock_data_3(self, data):
        ...
        if data is None:
            ...
            data.to_sql(name='eastmoney_zlpm_table', con=self.conn, if_exists = 'replace', index=False)
            pass
    ...
    def update_allstock_hdatadate(self, data):
        ...
        if data is None:
            ...
            length = len(data)
            sql_cmd = ""
            sql_head="UPDATE eastmoney_zlpm_table SET is_zig = tmp.is_zig, \
                    is_quad=tmp.is_quad, is_peach=tmp.is_peach FROM ( VALUES "

            sql_tail=" ) AS tmp (record_date, stock_code, is_peach, is_zig, is_quad ) \
                    WHERE eastmoney_zlpm_table.record_date = tmp.record_date \
                    and eastmoney_zlpm_table.stock_code = tmp.stock_code;"
    ...
    def delete_amount_is_zero(self):
        self.db_connect()
        sql_temp="delete from eastmoney_zlpm_table where amount = 0;"
    ...
    def get_data_from_hdata(self, stock_code=None, 
        ...
        sql_temp += "select"
        sql_temp += eastmoney_cols
        sql_temp += "from eastmoney_zlpm_table"
    ...
    def delete_data_from_hdata(self, stock_code=None, 
        ...
        sql_temp = "delete from eastmoney_zlpm_table"
...
#alter table eastmoney_zlpm_table add  "up_days" int not null default 0;


#update eastmoney_zlpm_table set is_zig=0 where record_date = '2018-10-08' and stock_code = '002732';

#UPDATE eastmoney_zlpm_table SET is_zig = tmp.is_zig, is_quad=tmp.is_quad FROM    (VALUES ( DATE  '2018-06-13', '600647', 11, 1), ( DATE  '2018-06-12', '600647', 12, 1) ) AS tmp (record_date, stock_code, is_zig, is_quad ) WHERE eastmoney_zlpm_table.record_date = tmp.record_date and eastmoney_zlpm_table.stock_code = tmp.stock_code;
#select * from eastmoney_zlpm_table where stock_code ='600647' and (record_date='2018-06-13' or record_date='2018-06-12' ); 
