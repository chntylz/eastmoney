...
### 股票代码    股票名称    记录日期    模式函数名  模式显示名  信号值  模式说明

pattern_cols = " stock_code, stock_name,  record_date, pat_func, pat_name, sig_value, pat_des " 


class HData_pattern(object):
    ...
    def table_is_exist(self):
        self.db_connect()
        self.cur.execute("select count(*) from pg_class where relname = 'pattern_table' ;")
    ...
    def db_hdata_pattern_create(self):
        ...
        alter table pattern_table add primary key(stock_code,record_date,pat_func);
            ''')
        self.conn.commit()
        self.db_disconnect()

        my_dbg("db_pattern_table_create finish")
        pass
    ...
    def copy_from_stringio(self, df):
        ...
        self.db_connect()
        try:
            self.cur.copy_from(buffer, table='pattern_table', sep=",")
            self.conn.commit()
    ...
    def db_get_maxdate_of_stock(self,stock_code):#获取某支股票的最晚日期
        ...
        self.db_connect()
        self.cur.execute("select max(record_date) from pattern_table \
            ...
            for i in range(0,length):
                ...
                if i % each_num == 0 and i != 0:
                    if debug:
                        my_dbg(sql_cmd)
                    if(sql_cmd != ""):
                        final_cmd = "insert into pattern_table ("\
                                + pattern_cols + \
                                " ) values "+sql_cmd+";"
                        if debug:
                            my_dbg(final_cmd)
        ...
        if data is None:
            ...
            for i in range(length):
                ...
                if i % each_num == 0 and i != 0:
                    ...
                    if len(sql_cmd):
                        final_sql = [] 
                        final_sql.append("insert into pattern_table (")
                        final_sql.append(pattern_cols)
    ...
    def insert_all_stock_data_3(self, data):
        ...
        if data is None:
            ...
            data.to_sql(name='pattern_table', con=self.conn, if_exists = 'replace', index=False)
            pass
    ...
    def delete_amount_is_zero(self):
        self.db_connect()
        sql_temp="delete from pattern_table where amount = 0;"
    ...
    def get_data_from_hdata(self, stock_code=None, 
        ...
        sql_temp = "select"  
        sql_temp += pattern_cols 
        sql_temp += "from ( "

        sql_temp += "select"
        sql_temp += pattern_cols
        sql_temp += "from pattern_table"
    ...
    def get_latest_data_from_hdata(self):#将数据库中的数据读取并转为dataframe格式返回
        ...
        self.db_connect()
        sql_temp = ' select * from pattern_table where record_date = '\
                + '(select max(record_date) from pattern_table as tmp_date); '
    ...
    def delete_data_from_hdata(self, stock_code=None, 
        ...
        sql_temp = "delete from pattern_table"
