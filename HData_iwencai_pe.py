...
iwencai_cols = " run_date, stock_code, record_date, pe_pct " 


class HData_iwencai_pe(object):
    def __init__(self,user,password):
        # self.aaa = aaa
        self.iwencai_pe_table=[]
        self.user=user
    ...
    def table_is_exist(self):
        self.db_connect()
        self.cur.execute("select count(*) from pg_class where relname = 'iwencai_pe_table' ;")
    ...
    def db_hdata_iwencai_create(self):

        self.db_connect()

        # 创建stocks表
        self.cur.execute('''
            drop table if exists iwencai_pe_table;
            create table iwencai_pe_table(
                run_date date,
                stock_code varchar,
                record_date date,
                pe_pct float
            );
        alter table iwencai_pe_table add primary key(stock_code,record_date);
            ''')
        self.conn.commit()
        self.db_disconnect()

        my_dbg("db_iwencai_pe_table_create finish")
        pass
    ...
    def copy_from_stringio(self, df):
        ...
        self.db_connect()
        try:
            self.cur.copy_from(buffer, table='iwencai_pe_table', sep=",")
            self.conn.commit()
    ...
    def db_get_maxdate_of_stock(self,stock_code):#获取某支股票的最晚日期
        ...
        self.db_connect()
        self.cur.execute("select max(record_date) from iwencai_pe_table \
            ...
            for i in range(0,length):
                ...
                if i % each_num == 0 and i != 0:
                    if debug:
                        my_dbg(sql_cmd)
                    if(sql_cmd != ""):
                        final_cmd = "insert into iwencai_pe_table ("\
                                + iwencai_cols + \
                                " ) values "+sql_cmd+";"
                        if debug:
                            my_dbg(final_cmd)
        ...
        if data is None:
            ...
            for i in range(length):
                ...
                if i % each_num == 0 and i != 0:
                    ...
                    if len(sql_cmd):
                        final_sql = [] 
                        final_sql.append("insert into iwencai_pe_table (")
                        final_sql.append(iwencai_cols)
    ...
    def insert_all_stock_data_3(self, data):
        ...
        if data is None:
            ...
            data.to_sql(name='iwencai_pe_table', con=self.conn, if_exists = 'replace', index=False)
            pass
    ...
    def delete_amount_is_zero(self):
        self.db_connect()
        sql_temp="delete from iwencai_pe_table where amount = 0;"
    ...
    def get_data_from_hdata(self, stock_code=None, 
        ...
        sql_temp = "select"  
        sql_temp += iwencai_cols 
        sql_temp += "from ( "

        sql_temp += "select"
        sql_temp += iwencai_cols
        sql_temp += "from iwencai_pe_table"
    ...
    def get_latest_data_from_hdata(self):#将数据库中的数据读取并转为dataframe格式返回
        ...
        self.db_connect()
        sql_temp = ' select * from iwencai_pe_table where record_date = '\
                + '(select max(record_date) from iwencai_pe_table as tmp_date); '
    ...
    def delete_data_from_hdata(self, stock_code=None, 
        ...
        sql_temp = "delete from iwencai_pe_table"
