...
if __name__ == '__main__':
    ...
    while (1):
        try:
            f_df, api_param = get_fina_data3('income', i)
            if len(f_df):
    ...
    df.to_csv('./csv/' + nowdate.strftime("%Y-%m-%d") +'_income_all_df_' + str(i) + '.csv', encoding='gbk')
    try :
...
'''
df = pd.read_csv('./csv/income_all_df_301.csv',encoding='gbk', index_col=[0], dtype={'security_code':str})
df = df.drop_duplicates(subset=['security_code', 'reportdate'], keep='first')
