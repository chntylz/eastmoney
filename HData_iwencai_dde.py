...
iwencai_cols = " record_date, rank, stock_code, stock_name, close, pct, \
                zlkp_pct, zlkp_rank, dde_buy, dde_sell, amount, dde_net, conti_day, dde_net_all "


class HData_iwencai_dde(object):
    def __init__(self,user,password):
        # self.aaa = aaa
        self.iwencai_dde_table=[]
        self.user=user
    ...
    def table_is_exist(self):
        self.db_connect()
        self.cur.execute("select count(*) from pg_class where relname = 'iwencai_dde_table' ;")
    ...
    def db_hdata_iwencai_create(self):
        ...
        # 创建stocks表
        self.cur.execute('''
            drop table if exists iwencai_dde_table;
            create table iwencai_dde_table(
                record_date date,
                rank float,
                stock_code varchar,
                stock_name varchar,
        ...
        my_dbg("db_iwencai_dde_table_create finish")
        pass
    ...
    def copy_from_stringio(self, df):
        ...
        self.db_connect()
        try:
            self.cur.copy_from(buffer, table='iwencai_dde_table', sep=",")
            self.conn.commit()
    ...
    def db_get_maxdate_of_stock(self,stock_code):#获取某支股票的最晚日期
        ...
        self.db_connect()
        self.cur.execute("select max(record_date) from iwencai_dde_table \
            ...
            for i in range(0,length):
                ...
                if i % each_num == 0 and i != 0:
                    if debug:
                        my_dbg(sql_cmd)
                    if(sql_cmd != ""):
                        final_cmd = "insert into iwencai_dde_table ("\
                                + iwencai_cols + \
                                " ) values "+sql_cmd+";"
                        if debug:
                            my_dbg(final_cmd)
        ...
        if data is None:
            ...
            for i in range(length):
                ...
                if i % each_num == 0 and i != 0:
                    ...
                    if len(sql_cmd):
                        final_sql = [] 
                        final_sql.append("insert into iwencai_dde_table (")
    ...
    def insert_all_stock_data_3(self, data):
        ...
        if data is None:
            ...
            data.to_sql(name='iwencai_dde_table', con=self.conn, if_exists = 'replace', index=False)
            pass
    ...
    def delete_amount_is_zero(self):
        self.db_connect()
        sql_temp="delete from iwencai_dde_table where amount = 0;"
    ...
    def get_data_from_hdata(self, stock_code=None, 
        ...
        sql_temp += "select"
        sql_temp += iwencai_cols
        sql_temp += "from iwencai_dde_table"
    ...
    def get_latest_data_from_hdata(self):#将数据库中的数据读取并转为dataframe格式返回
        ...
        self.db_connect()
        sql_temp = ' select * from iwencai_dde_table where record_date = '\
                + '(select max(record_date) from iwencai_dde_table as tmp_date); '
    ...
    def delete_data_from_hdata(self, stock_code=None, 
        ...
        sql_temp = "delete from iwencai_dde_table"
