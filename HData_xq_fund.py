...
xq_cols = ' stock_code, record_date, to_float_shares_ratio, org_name_or_fund_name, held_num '

class HData_xq_fund(object):
    ...
    def db_hdata_xq_create(self):
        ...
        # 创建stocks表
        self.cur.execute('''
            drop table if exists xq_fund_table;
            create table xq_fund_table(
                stock_code varchar,
                record_date  date,
                to_float_shares_ratio float,
                org_name_or_fund_name  varchar,
                held_num float
                );
            alter table xq_fund_table add primary key(stock_code,record_date);
            ''')
        self.conn.commit()
        self.db_disconnect()
    ...
    def db_get_maxdate_of_stock(self,stock_code):#获取某支股票的最晚日期
        ...
        self.db_connect()
        self.cur.execute("select max(record_date) from xq_fund_table \
    ...
    def get_data_from_hdata(self, stock_code=None, 
        ...
        sql_temp += "select * from xq_fund_table"
    ...
    def delete_data_from_hdata(self, stock_code=None, 
        ...
        sql_temp = "delete from xq_fund_table"
...
#alter table xq_fund_table add  "up_days" int not null default 0;


#update xq_fund_table set is_zig=0 where record_date = '2018-10-08' and stock_code = '002732';

#UPDATE xq_fund_table SET is_zig = tmp.is_zig, is_quad=tmp.is_quad FROM    (VALUES ( DATE  '2018-06-13', '600647', 11, 1), ( DATE  '2018-06-12', '600647', 12, 1) ) AS tmp (record_date, stock_code, is_zig, is_quad ) WHERE xq_fund_table.record_date = tmp.record_date and xq_fund_table.stock_code = tmp.stock_code;
#select * from xq_fund_table where stock_code ='600647' and (record_date='2018-06-13' or record_date='2018-06-12' ); 
