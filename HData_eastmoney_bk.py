...
eastmoney_cols = " record_date, zxj , percent , chg , turnoverrate , stock_code , stock_name , \
        mkt_cap , up_num , down_num , lz_name , lz_code , lz_percent , last_name , last_code , last_percent, zig  "

class HData_eastmoney_bk(object):
    def __init__(self,user,password):
        # self.aaa = aaa
        self.eastmoney_bk_table=[]
        self.user=user
    ...
    def table_is_exist(self):
        self.db_connect()
        self.cur.execute("select count(*) from pg_class where relname = 'eastmoney_bk_table' ;")
    ...
    def db_hdata_eastmoney_create(self):
        ...
        # 创建stocks表
        self.cur.execute('''
            drop table if exists eastmoney_bk_table;
            create table eastmoney_bk_table(
                record_date date, 
                zxj float, 
                percent float,
                chg float,
        ...
        alter table eastmoney_bk_table add primary key(stock_code,record_date);
            ''')
        self.conn.commit()
        self.db_disconnect()

        my_dbg("db_eastmoney_bk_table_create finish")
        pass
    ...
    def copy_from_stringio(self, df):
        ...
        self.db_connect()
        try:
            self.cur.copy_from(buffer, table='eastmoney_bk_table', sep=",")
            self.conn.commit()
    ...
    def db_get_maxdate_of_stock(self,stock_code):#获取某支股票的最晚日期
        ...
        self.db_connect()
        self.cur.execute("select max(record_date) from eastmoney_bk_table \
            ...
            for i in range(0,length):
                ...
                if i % each_num == 0 and i != 0:
                    if debug:
                        my_dbg(sql_cmd)
                    if(sql_cmd != ""):
                        final_cmd = "insert into eastmoney_bk_table ("\
                                + eastmoney_cols + \
                                " ) values "+sql_cmd+";"
                        if debug:
                            my_dbg(final_cmd)
        ...
        if data is None:
            ...
            for i in range(length):
                ...
                if i % each_num == 0 and i != 0:
                    ...
                    if len(sql_cmd):
                        final_sql = [] 
                        final_sql.append("insert into eastmoney_bk_table (")
    ...
    def insert_all_stock_data_3(self, data):
        ...
        if data is None:
            ...
            data.to_sql(name='eastmoney_bk_table', con=self.conn, if_exists = 'replace', index=False)
            pass
    ...
    def delete_amount_is_zero(self):
        self.db_connect()
        sql_temp="delete from eastmoney_bk_table where amount = 0;"
    ...
    def get_data_from_hdata(self, stock_code=None, 
        ...
        sql_temp += "select"
        sql_temp += eastmoney_cols
        sql_temp += "from eastmoney_bk_table"
    ...
    def get_latest_data_from_hdata(self):#将数据库中的数据读取并转为dataframe格式返回
        ...
        self.db_connect()
        sql_temp = ' select * from eastmoney_bk_table where record_date = '\
                + '(select max(record_date) from eastmoney_bk_table as tmp_date); '
